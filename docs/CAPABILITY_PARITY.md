...
| Skills auto-refresh          | Startup hook, `/update-skills`, git hooks (`/install-git-hooks`) | Session-start hook                | Session-start hook               | `UIPATH_SKILLS_AUTO_REFRESH=0` to disable. Updater force-resets to `origin/main`; local drift moved to a `backup/local-<ts>` branch. |
