  ...
  return (
    <button onClick={onClick} style={{
      ...
      {count !== undefined && count > 0 && (
        <span style={{ fontSize: 9, color: PALETTE.textMute, background: PALETTE.bg, padding: "1px 5px", borderRadius: 8, fontWeight: 500 }}>
          {count}
        </span>
      )}
    </button>
  ...
  return (
    <div style={{
      ...
      <button onClick={toggle} style={{
        ...
        <div style={{
          fontFamily: "'JetBrains Mono', monospace",
          fontSize: 10, color: PALETTE.textDim, letterSpacing: "0.08em",
          ...
          {c.score !== undefined && (
            <span style={{ marginLeft: "auto", fontSize: 9, color: PALETTE.textMute }}>
              score {c.score}
            </span>
          )}
        </div>
      ...
      {expanded && (
        <div style={{
          ...
          {loading && (
            <div style={{ color: PALETTE.textMute, fontStyle: "italic", fontFamily: "'JetBrains Mono', monospace", fontSize: 10 }}>
              loading section…
            </div>
          )}
          {error && (
            <div style={{ color: "#92400e", fontFamily: "'JetBrains Mono', monospace", fontSize: 10 }}>
              {error}
            </div>
          )}
          {body !== null && (
...
function SkillBodyTab({ skillId }: { skillId: string }) {
  ...
  return (
    <div style={{ padding: 18, fontFamily: "'Inter', sans-serif" }}>
      ...
      {loading && (
        <div style={{ marginTop: 10, color: PALETTE.textMute, fontSize: 11, fontFamily: "'JetBrains Mono', monospace" }}>
          loading skill context...
        </div>
      )}
      {error && (
        <div style={{ marginTop: 10, color: "#92400e", fontSize: 11, fontFamily: "'JetBrains Mono', monospace" }}>
          {error}
        </div>
      )}
      {detail?.body && (
...
function SkillCoverageTab({ node, graph, onSelectNode }: {
  ...
  return (
    <div style={{ padding: 18 }}>
      ...
      <div style={{ marginTop: 10, display: "flex", flexDirection: "column", gap: 6 }}>
        {matched.length === 0 ? (
          <div style={{ color: PALETTE.textMute, fontSize: 11, fontFamily: "'JetBrains Mono', monospace" }}>
          ...
          return (
            ...
            >
              ...
              <div style={{ flex: 1, minWidth: 0 }}>
                ...
                <div style={{ fontSize: 9, color: layer.color, fontFamily: "'JetBrains Mono', monospace", letterSpacing: "0.14em", marginTop: 2 }}>
      ...
      <div style={{ marginTop: 12, fontSize: 11, color: PALETTE.textDim, lineHeight: 1.5 }}>
    ...
    <div style={{ padding: 18, fontFamily: "'Inter', sans-serif" }}>
      ...
      {(node.business_status || node.business_meta) && (
        <div style={{
          ...
          {node.business_meta && (
            <div style={{ marginTop: 8, display: "grid", gridTemplateColumns: "1fr 1fr", gap: "4px 12px", fontSize: 11, fontFamily: "'JetBrains Mono', monospace" }}>
          ...
          <div style={{ marginTop: 10, display: "flex", flexDirection: "column", gap: 6 }}>
            {errors.map((e, i) => (
              <div key={i} style={{
                ...
                <div style={{ fontSize: 11, lineHeight: 1.5, color: e.severity === "error" ? "#991b1b" : e.severity === "warn" ? "#92400e" : "#1e40af" }}>
                  {e.message}
                </div>
              </div>
            ))}
      ...
      {node.meta && Object.keys(node.meta).length > 0 && (
        <div style={{ marginTop: 22 }}>
          <Section label="META" />
          <div style={{ marginTop: 10, fontSize: 11, lineHeight: 1.9, fontFamily: "'JetBrains Mono', monospace" }}>
  ...
  return (
    <div style={{ padding: 18, fontFamily: "'JetBrains Mono', monospace" }}>
      ...
      <div style={{
        ...
        <div style={{ flex: 1, fontSize: 11.5, color: PALETTE.text, wordBreak: "break-all" }}>
          {node.code.path}
        </div>
        {node.code.lines && (
          <div style={{ fontSize: 10, color: PALETTE.textDim, background: PALETTE.panel, padding: "2px 7px", borderRadius: 3, border: `1px solid ${PALETTE.rule}`, letterSpacing: "0.05em" }}>
            L {node.code.lines}
          </div>
        )}
      </div>
      ...
      {node.code.snippet && (
        <div style={{ marginTop: 18 }}>
          ...
          <div style={{ marginTop: 10, background: "#1e1e1e", border: `1px solid ${PALETTE.rule}`, borderRadius: 6, overflow: "hidden" }}>
            <div style={{ padding: "6px 12px", borderBottom: "1px solid #2d2d2d", fontSize: 9, letterSpacing: "0.2em", color: "#858585", fontWeight: 600, display: "flex", alignItems: "center", gap: 6 }}>
            ...
            <pre style={{ margin: 0, padding: "12px 14px", fontSize: 11.5, lineHeight: 1.55, color: "#d4d4d4", fontFamily: "'JetBrains Mono', monospace", overflowX: "auto", whiteSpace: "pre" }}>
...
function KnowledgeTab({ node, worktreeId }: { node: ProjectNode; worktreeId: string }) {
  ...
  useEffect(() => {
    ...
    setLoading(true);
    setError(undefined);
    loadNodeKnowledge(worktreeId, node.id, node.label).then((res) => {
      if (cancelled) return;
  ...
  return (
    <div style={{ padding: 18, fontFamily: "'Inter', sans-serif" }}>
      <div style={{
        marginBottom: 14,
        fontFamily: "'JetBrains Mono', monospace",
        fontSize: 9, letterSpacing: "0.18em", color: PALETTE.textMute,
    ...
    if (edges.length === 0) {
      return (
        <div style={{ marginBottom: 18 }}>
          ...
          <div style={{ marginTop: 8, fontSize: 11, color: PALETTE.textMute, fontFamily: "'JetBrains Mono', monospace" }}>
            ∅ none
          </div>
        </div>
      );
              ...
              <div key={edge.id} style={{ display: "flex", gap: 4 }}>
                ...
                >
                  ...
                  <div style={{ flex: 1, minWidth: 0 }}>
                    ...
                    <div style={{ fontSize: 9, color: style.color, fontFamily: "'JetBrains Mono', monospace", letterSpacing: "0.15em", fontWeight: 600, marginTop: 2 }}>
...
export default function Inspector({
  ...
  if (edge && !node) {
    ...
    return (
      <div style={{
        width: 360, background: PALETTE.panel,
        borderLeft: `1px solid ${PALETTE.rule}`,
        display: "flex", flexDirection: "column",
        overflow: "hidden", fontFamily: "'Inter', sans-serif", flexShrink: 0,
      }}>
        <div style={{ padding: 18, borderBottom: `1px solid ${PALETTE.rule}`, display: "flex", alignItems: "center", gap: 8 }}>
          <div style={{ flex: 1 }}>
            <div style={{
              fontSize: 9, letterSpacing: "0.22em", color: style.color,
            ...
            <div style={{ fontSize: 14, color: PALETTE.text, fontWeight: 600 }}>
            ...
            <div style={{ marginTop: 10, display: "flex", flexDirection: "column", gap: 6 }}>
              {fromNode && (
                <button onClick={() => onSelectNode(fromNode.id)} style={endpointBtn(getLayer(fromNode.layer).color)}>
                  <span style={{ fontSize: 9, color: PALETTE.textDim, fontFamily: "'JetBrains Mono', monospace", letterSpacing: "0.15em" }}>FROM</span>
              ...
              {toNode && (
                <button onClick={() => onSelectNode(toNode.id)} style={endpointBtn(getLayer(toNode.layer).color)}>
                  <span style={{ fontSize: 9, color: PALETTE.textDim, fontFamily: "'JetBrains Mono', monospace", letterSpacing: "0.15em" }}>TO</span>
  ...
  if (!node) {
    return (
      <div style={{
        ...
        <div style={{
          padding: "10px 16px", borderBottom: `1px solid ${PALETTE.rule}`,
    ...
    <div style={{
      ...
      <div style={{ padding: 18, borderBottom: `1px solid ${PALETTE.rule}` }}>
        <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 12 }}>
          <div style={{ width: 36, height: 36, borderRadius: 6, background: layer.soft, border: `1px solid ${layer.color}33`, display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}>
            {KindIcon && <KindIcon size={18} color={layer.color} strokeWidth={1.8} />}
          </div>
          <div style={{ flex: 1, minWidth: 0 }}>
            <div style={{ fontSize: 9, letterSpacing: "0.22em", color: layer.color, fontWeight: 700, fontFamily: "'JetBrains Mono', monospace", marginBottom: 2, display: "flex", alignItems: "center", gap: 6 }}>
            ...
            <div style={{ fontSize: 16, color: PALETTE.text, fontWeight: 600, wordBreak: "break-all", lineHeight: 1.2 }}>
              {node.label}
            </div>
          </div>
          <button onClick={onToggleCollapsed} title="Collapse inspector" style={{
        ...
        <div style={{
          ...
          padding: "3px 7px", background: PALETTE.bg,
        ...
        {node.children && node.children.nodes.length > 0 && (
          ...
          >
            ...
            <span style={{ background: "rgba(0,0,0,0.08)", padding: "2px 7px", borderRadius: 3, fontSize: 10 }}>
