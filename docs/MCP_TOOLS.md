...
Each tool has an **audience guide** (synthesized from the registered `Tool` metadata), the **author registration text** (verbatim `Tool.description` from code), the JSON **input schema**, and a **behavior flow** Mermaid chart that reflects the real dispatch path (see `framework/mcp_server/tools/*_tools.py` and `call_*_tool`). Diagrams follow the Pro Standard: neutral `%%{init}%%`, `classDef` node colors, muted `linkStyle`.
