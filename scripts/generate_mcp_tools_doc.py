...
Run from repo root:
  python scripts/generate_mcp_tools_doc.py
...
def build_markdown() -> str:
    ...
    lines.append("")
    lines.append(
        "This file is generated by `scripts/generate_mcp_tools_doc.py`. "
        "Regenerate after changing tool definitions in `mcp_server/tools/`."
    )
    lines.append("")
    ...
    lines.append(
        ...
        "(see `mcp_server/tools/*_tools.py` and `call_*_tool`). Diagrams follow "
    ...
    lines.append("")
    lines.append(
        "1. Implement `Tool(...)` in the appropriate `mcp_server/tools/*.py` "
        "and wire `call_*_tool` dispatch."
    )
    lines.append(
        "2. Register the name branch in "
        "[`mcp_server/server.py`](../mcp_server/server.py) `call_tool`."
    )
    lines.append(
        "3. Add `ToolAnnotations` (read-only vs destructive vs staging) and "
        "extend [`tests/mcp/test_tool_annotations.py`](../tests/mcp/test_tool_annotations.py) "
        "so `READ_ONLY` / `DESTRUCTIVE` / `STAGING` covers every tool."
    )
    lines.append(
        "4. Add or adjust the tool-specific Mermaid body in "
        "[`scripts/mcp_tools_doc_diagrams.py`](../scripts/mcp_tools_doc_diagrams.py) "
        "(`diagram_body_for_tool`) so generated docs match real behavior."
    )
    lines.append("5. Re-run `python scripts/generate_mcp_tools_doc.py`.")
    lines.append("")
    for mod, getter in MODULE_GETTERS:
        ...
        for tool in getter():
            ...
            lines.append("")
            lines.append(
                f"**Dispatch:** [`mcp_server/tools/{mod}_tools.py`](../mcp_server/tools/{mod}_tools.py) "
