...
> **Who this is for:** AI coding assistants (Codex, Cursor, Windsurf, Copilot) working in UiPath repositories.
...
| Contribution rules for skills themselves | `skills/AGENTS.md` |
| Files that `uipath init` generates in an agent project (`AGENTS.md`, `.agent/*`, `AGENTS.md` inside the agent) | Generated on demand by the Python `uipath` CLI |
...
- Never duplicate content from a `SKILL.md` into this `AGENTS.md` - reference the skill id instead.
...
- When `uipath init` emits its own `AGENTS.md` / `AGENTS.md` inside a generated agent folder, **do not overwrite** this repo-level `AGENTS.md`. Pass `--no-agents-md-override` or keep the generated files under the agent subfolder.
...
| Any Python agent work | Cursor / Codex + `uipath` CLI |
...
1. **Run project discovery.** If `.Codex/rules/project-context.md` doesn't exist, invoke the `uipath-project-discovery-agent` from `skills/agents/` to produce it. This populates project identity, dependencies, entry points, conventions.
...
These are the defaults this rule enforces. Project-specific conventions belong in a project-local `AGENTS.md` addendum or in `.Codex/rules/project-context.md` produced by the discovery agent.
...
- Changing `.cursorrules`, `AGENTS.md`, or `docs/uipath-*.md`.
...
- `uipath new <name>` + `uipath init` - scaffolded a minimal LangGraph agent. Observed:
  ...
  - `uipath init` emits a `AGENTS.md` / `AGENTS.md` inside the agent folder by default; pass `--no-agents-md-override` to preserve your own.
