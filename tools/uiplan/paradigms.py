...
def paradigm_task_blocks(paradigm: str) -> str:
    ...
    if paradigm == "solution":
        return base + (
            ...
            "Record `projects/<Project>/project.json`, generated `projects/<Project>/Main.xaml`, "
            "and template provenance for each. "
            ...
            "using **UiPath activities** from `plan.md` Activity Inventory (resolved via "
            "`uipath_doc_get_activity`); wire queue Add Queue Item activity, correlationId LogMessage, "
            "and connector inputs. Verify `uipcli package analyze projects/<Dispatcher>/project.json "
            "--resultPath out/analyze-dispatcher.json`. [skill:uipath-rpa] "
            "[library:uipath_doc_get_activity] [library:uipath_library_search]\n"
            "- [ ] T011C [US1] Implement performer / Long-Running orchestrator in "
            "`projects/<AnalyzerRunner>/Main.xaml` (workflow type from `plan.md` Workflow Catalog) "
            "with Get Queue Item, Invoke Coded Agent (host boundary; declare request/response "
            "schema), Set Status, and correlation-aware logging. "
            ...
            "with `uv run pytest` + `uipath run` fixture. [skill:uipath-agents] "
            "[library:uipath_library_search] [askai:query_uipath_docs]\n"
            "- [ ] T011E [US1] Implement **Custom HITL** surface in "
            "`projects/<HumanReview>/Main.xaml` (Sequence) using Action Center External Tasks + "
            "the org HITL_Application (Adaptive Cards + Slack approvals; see "
            "[`templates/uiplan/_workflow-catalog.md`](../../templates/uiplan/_workflow-catalog.md) "
            "and `[skill:uipath-custom-hitl]`). Implement Create External Task activity with payload "
            "schema, Slack notification path, Wait For External Task And Resume, and outcome handling. "
            "Do **not** use UiPath Flow for HITL. "
            "[skill:uipath-custom-hitl] [skill:uipath-rpa] [library:uipath_library_search]\n"
