...
Do **NOT** activate (redirect to `uipath-governance` instead) when the user asks about:
...
```bash
uip llm-configuration byo-connections list-product-configs --product jarvis --feature jarvis-natural-language-query --models-only --output json
```
